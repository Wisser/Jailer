...
public class TableDetailsView extends javax.swing.JPanel {
			...
			public void run() {
				...
				for (int i = 0; i < columns.size(); ++i) {
					if (row != null && row.values.length > i) {
						...
						if (v == null) {
							columnValue.put(columns.get(i), null);
						} else {
							String iltv = UIUtil.indicateLeadingAndTrailingSpaces(v.toString(), columns.get(i));
							if (UIUtil.isCHARType(columns.get(i))) {
								iltv = iltv.trim();
							}
							columnValue.put(columns.get(i), iltv);
						}
					}
				}
    ...
    private void initComponents() {
        ...
        setBackground(/* Renaming also in *.form! */ Colors.Color_255_255_255);
