...
WHERE T.TABSCHEMA=UPPER(''{0}'') AND T.TYPE=''T'' AND PK.KEYSEQ IS NOT NULL AND NOT T.TABNAME IN (''JL_DEPENDENCY'', ''JL_ENTITY'', ''JL_GRAPH'', ''JL_SET'')
...
WHERE T.TABSCHEMA=UPPER(''{0}'') AND T.TYPE=''T'' AND NOT T.TABNAME IN (''JL_DEPENDENCY'', ''JL_ENTITY'', ''JL_GRAPH'', ''JL_SET'') AND NOT EXISTS (SELECT * FROM syscat.COLUMNS PK WHERE PK.TABNAME=T.TABNAME AND PK.TABSCHEMA=T.TABSCHEMA AND PK.KEYSEQ IS NOT NULL)
